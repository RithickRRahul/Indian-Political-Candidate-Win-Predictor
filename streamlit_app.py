...
def load_models():
    ...
    try:
        # Load only the necessary model and columns
        model = joblib.load("gradient_boosting.pkl")
        expected_columns = joblib.load("expected_columns.pkl")
    except FileNotFoundError as e:
        st.error(f"Error loading model or column file: {e}")
        st.error("Ensure 'gradient_boosting.pkl' and 'expected_columns.pkl' are in the correct directory.")
        st.stop()
...
try:
    # MODIFIED: Load only the single model and columns
    model, expected_columns = load_models()
    party_win_rates, gender_win_rates, party_gender_win_rates = load_stats()

    # --- Prediction Section ---
    st.subheader("Make a Prediction")

    # Use columns for layout - simplified to stack inputs
    # col1_input, col2_input = st.columns(2) # Removed two-column layout for inputs

    # with col1_input: # Removed column context
    ...
    # MODIFIED: Removed Model Selection UI
    # with col2_input:
    #     st.markdown("#### Model Selection")
    #     model_keys = list(models.keys())
    #     ... (selectbox code removed) ...

    # NEW: Display fixed model name above the button
    st.caption("Model used: Gradient Boosting")

    predict_button = st.button("🔮 Predict Outcome", use_container_width=True, type="primary")

    # --- Prediction Results Display ---
    # This block will now appear directly below the button if clicked
    if predict_button:

        # MODIFIED: Removed RF Warning
        # if selected_model == "Tuned RF":
        #     st.warning(...)

        with st.spinner("Analyzing election data..."):
            fixed_model_name = "Gradient Boosting" # Hardcoded model name

            # Store inputs for potential later use
            st.session_state.last_input = {
                "party": party, "gender": gender, "model": fixed_model_name,
            ...
            # MODIFIED: Select model directly (using the pre-loaded 'model' variable)
            # model = models[selected_model] # No longer needed
    ...
    **Key Insights:**
    ...
    with col2_foot:
        st.caption("App Version 3.0 - Fixed Model") # Increment version

except FileNotFoundError as e:
     st.error(f"Error loading required files: {e}")
     st.error("Please ensure 'gradient_boosting.pkl' and 'expected_columns.pkl' are in the same directory as the Streamlit app.")
